...
#test zero mean and normality of residuals
hist(residuals)
...
## Nested F-tests
Our model is built off the relationship between median household income and share of adults 25 and older with a high-school degreee. The following nested F-tests were used to see if the additional variables (share_vote_trump and share_white_poverty) strengthen the model.
```{r}
...
```
Both nested F-tests are significant, so the additional variables are necessary and strengthen our model.
...
summary(model)
```
78.89% of the variance in median household income is explained by our model with the variables share_pop_hs, share_vote_trump, and share_white_poverty.
