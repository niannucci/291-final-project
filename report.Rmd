...
The response variable that we will be looking at is median_household_inc, which is the median household income for a state in 2016 in USD. None of the values are excessively large or small, ranging between $35,521 and \$76,165.
...
## Checking conditions
``` {r}
#test linearity, homoskedascity, zero mean, and normality of residuals
...
cor(share_white_poverty, share_pop_hs)
cor(share_white_poverty, share_vote_trump)
cor(share_pop_hs, share_vote_trump)
...
# Results
Our results show that the degree of high school education, white poverty, and share of votes for Trump of a population possess a high degree of joint explanatory power for median household income. 

To test the assumptions of Multiple Linear Regression, we perform several diagnostic tests for linearity, independence, homoskedascity, and normality and zero mean of residuals.

To test for multicollinearity, we employed F-tests and correlation tests. The Pearson correlation coefficient for the predictors "Share of Whites in Poverty" and "Share of Population with High School Diploma" is -0.49, the correlation coefficient for the predictors "Share of Whites in Poverty" and "Share of Population that Voted for Trump" is 0.55, correlation coefficient for the predictors "Share of Population with High School Diploma" and "Share of Population that Voted for Trump" is -0.15. Between all of our predictors, the highest magnitude Pearson correlation coefficient is 0.55, which indicates only moderate positive linear correlation. Therefore we elect to keep all predictors in the final model in conjunction with the F tests of joint significance.
